...
export function HireUsPageContent() {
  ...
  const filteredTalent =
    selectedFilter === "All Talent"
      ...
      : talent.filter(
          (person) =>
            person.availability?.toLowerCase() === selectedFilter.toLowerCase(),
        );

  return (
    <div className="relative min-h-screen bg-[#0f0f0f] py-24 font-sans">
      <div className="relative z-10 mx-auto max-w-7xl px-6 lg:px-12 pt-12">
        ...
        <div className="text-center mb-16">
          <h1 className="text-4xl sm:text-6xl lg:text-7xl font-black tracking-tight text-white font-sans leading-tight">
            ...
            <Highlighter action="underline" color="#F47174" strokeWidth={4}>
              <span className="text-[#F47174]">Talent.</span>
            </Highlighter>
          </h1>
          <p className="mt-6 text-base sm:text-lg text-[#8C93B0] max-w-3xl mx-auto font-sans leading-relaxed">
            Discover top-tier engineers, designers, and developers from KNIT
            Sultanpur actively seeking internships, full-time roles, and
            freelance opportunities.
          </p>

          <div className="mt-8">
            <a
              href="mailto:contact@ptsc.knit.ac.in"
              className="inline-flex items-center gap-2 bg-white text-black font-black uppercase tracking-wider px-8 py-3.5 rounded-full hover:bg-white/90 transition-all shadow-[0_0_20px_rgba(255,255,255,0.2)] hover:shadow-[0_0_30px_rgba(255,255,255,0.4)] hover:-translate-y-1"
            >
              ...
              <button
                ...
                className={`relative rounded-full px-6 py-2.5 text-xs font-black uppercase tracking-[0.1em] transition-all duration-300 ${
                  selectedFilter === filter
                    ? "bg-[#F47174] text-white shadow-[0_0_20px_rgba(244,113,116,0.3)]"
        ...
        {loading ? (
          <div className="p-16 text-center border border-white/10 rounded-3xl bg-[#121528] text-white/50 text-xs font-mono flex items-center justify-center gap-2">
            <FiLoader className="size-5 animate-spin text-[#F47174]" /> Loading
            talent directory...
          </div>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 font-sans">
            {filteredTalent.map((person) => {
              const availabilityColor =
                person.availability === "Internship"
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-4 border-b border-white/10 pb-5 mb-5">
                    <div className="relative size-16 shrink-0 overflow-hidden rounded-full border-2 border-white/10 group-hover:border-[#F47174] transition-colors bg-[#0B0D19]">
                      <Image
                    ...
                    <div>
                      <h3 className="text-xl font-black text-white tracking-tight group-hover:text-[#F47174] transition-colors">
                        {person.name}
                      </h3>
                  ...
                  <div className="mb-4">
                    <span
                      className={`inline-block px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-full border ${availabilityColor}`}
                    >
                  ...
                  <div className="mb-6 flex-1">
                    <p className="text-xs font-bold text-[#8C93B0] uppercase tracking-wider mb-2">
                      Top Skills
                    </p>
                    <div className="flex flex-wrap gap-2">
                      {person.skills.map((skill) => (
                        <span
                          key={skill}
                          className="px-2.5 py-1 text-xs font-medium text-white/80 bg-white/5 border border-white/10 rounded-md"
                        >
                          {skill}
                        </span>
                      ))}
                  ...
                  <div className="mt-auto pt-4 border-t border-white/10 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      {person.linkedin && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-white/50 hover:text-[#0077b5] transition-colors p-1"
                          title="LinkedIn"
                        >
                    ...
                    {person.resume && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-1.5 text-xs font-bold text-[#F47174] uppercase tracking-wider hover:text-white transition-colors"
                      >
